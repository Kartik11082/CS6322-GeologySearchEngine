"""
Metric clustering analysis — writes expander/textfiles/Metric/q{1,2,3}_*.txt
...
def main() -> None:
    ...
    for q_idx, query in enumerate(METHOD_ANALYSIS_QUERIES, start=1):
        ...
        corr = format_metric_correlations(ctx, expander)
    ...
    header = "\n".join(
        [
            "=" * 80,
            "METRIC CLUSTERING - DETAILED ANALYSIS (merged from textfiles/)",
            "Student: Uddesh Singh | NetID: UXS230004",
            "=" * 80,
            "",
            "Formula: C(u,v) = Σ [ 1 / |pos(u) - pos(v)| ] over text_preview token positions.",
            "",
        ]
    )
    report_path = merge_textfiles_into_report(
        METHOD, header, "report_METRIC_clustering.txt"
    )
