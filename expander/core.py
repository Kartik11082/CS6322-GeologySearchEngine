...
class QueryExpander:
    ...
    def _get_local_doc_set(self, query: str, top_k: int) -> set[str]:
        results = self.engine.search(query, method="bm25", top_k=top_k)
    ...
    # =====================================================================
    def expand_association(self, query: str, top_k_docs: int = 50, m_neighbors: int = 2, normalized: bool = False) -> str:
        """
        ...
        query_stems = preprocess(query)
        expanded_query_terms = set(query.split()) # Keep original casing
        ...
        for q_term in query_stems:
            ...
            for v_term, v_postings in local_tf.items():
                if v_term in query_stems: continue
            ...
            top_neighbors = sorted(correlations.items(), key=lambda x: x[1], reverse=True)[:m_neighbors]
            for neighbor, _ in top_neighbors:
                expanded_query_terms.add(neighbor)
    ...
    #  3. SCALAR CLUSTERS
    # =====================================================================
    def expand_scalar(self, query: str, top_k_docs: int = 50, m_neighbors: int = 2) -> str:
        """
        ...
        expanded_query_terms = set(query.split())
    ...
    #  4. METRIC CLUSTERS
    # =====================================================================
    def expand_metric(self, query: str, top_k_docs: int = 50, m_neighbors: int = 2) -> str:
        """
        ...
        for q_term in query_stems:
            ...
            top_neighbors = sorted(global_correlations[q_term].items(), key=lambda x: x[1], reverse=True)[:m_neighbors]
            for neighbor, _ in top_neighbors:
                expanded_query_terms.add(neighbor)
