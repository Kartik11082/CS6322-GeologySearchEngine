...
*(Note: While `GET /search?q=...` is typical for simple search endpoints, `POST` is highly recommended when using FastAPI as it cleanly allows Pydantic to validate complex schemas or extended body parameters without messing up the URI length.)*
